...
There are three approaches to reducing IDS (strict, advanced and minmax).
	...
	\begin{figure}
		\begin{algorithm}[H]
			...
			Neighbours $N$ $\leftarrow$ \RangeQuery{DB, dist, p, $\varepsilon$}\Comment*[f]{Find initial neighbors}
			...
			\ForEach{$q$ in $S$}{
				...
				Neighbours $N$ $\leftarrow$ \RangeQuery{DB, dist, q, $\varepsilon$}
...
\chapter{Conclusion}
Nowadays when we want to block malicious software we can use some software which
...
call which was not allowed. This is done by seccomp and when you want to use it
you must write the policy by hand.

The goal of this thesis was to automate this process by translating strace to
seccomp. This thesis is divided into multiple parts such as designing and
implementing the intermediate data structure (IDS), used to represent system
call in this tool. Another was to design and implement optimization algorithms
for IDS. There were implemented three of them (strict, weak and advanced). The
implementation was designed with design patterns to be easily extended with new
optimization algorithms. Another part was to implement a policy generator from
IDS. We decided to implement the policy generator into C/C++. In the end of the
thesis, we evaluated the created policy. Multiple issues were found which
were not caused by faulty implementation, but they were caused by the
interpretation in seccomp. We analyzed this issue in the last part of this
thesis.

This thesis can be extended with an interactive tool that generates a
filter, runs the testsuite for a specific program with that filter and
interactively edits issues found in the generated policy. Other extensions can be
an implementation of other clustering algorithms or output into other languages
as Go or Python that can be done as another dedicated thesis.
