...
In this case the acceptance testing is used even on policy generator testing
which means generate policies, i.e, whitelist of syscalls from strace of given
program and applying the whitelist into carefully picked programs. Afterwards we
run the testsuite provided for that program and by this way we can immediately
see if the generated whitelist works correctly.

\subsection{Testing on real programs}
I decided for four programs which will appear in acceptance testing. One is
demanded by RedHat Inc. and that is
USBGuard\footnote{\url{https://usbguard.github.io/}}. USBGuard is open source
project developed under GNU GPL v2.0 licence. It is implementinf USB device
authorization which can be set up localy or over centralised management.
\cite{usbguardCentralized} Analyzing a gathered system calls, from USBGuard we
can say that it is a complex program. And this is really needed for quality
acceptance testing. Another advantage of USBGuard is that it has a good
testsuite which will be helpfull in evaluation of generated policies by
\texttt{strace2seccomp}. We will focus only on \texttt{usbguard-daemon} which is
the main program of whole project.
...
The \emph{cp} is a program used for copying files acros filesystem on GNU Linux
operating systems.One of the benefits of GNU projects is that they have a
relatively big testsuite. The drawbacks of this program is that it is part of a
big project and is though to acquire strace logs from testsuite. This problem I
solved by manualy patching the Makefile which was responsible for runing tests.
The patch added before \texttt{cp} command the strace command with appropriate
flags mentioned above in Section \ref{strace_params}.

Another program is \emph{find} from GNU project called
\emph{findutils}\footnote{\url{https://www.gnu.org/software/findutils/}}. The
\emph{find} is a program used to find files or directories in filesystem. It is
specific for GNU Linux as well as \emph{cp}. This program has really good
testsuite like \emph{coreutils} project. The drawback of complexity is simmilar
\emph{coreutils} as well. Find was chosen for high number of syscalls which are
different only in a provided arguments. This will test the robustness of
clustering in \texttt{strace2seccomp} tool.

The last program is my school project from \textit{Network Applications} class
called \emph{testovac}\footnote{\url{https://github.com/tammar96/ISA-testovac}}.
This project was chosen for its architecture. The testovac is multithreaded
application and here we can test if the filter is successfuly distributed on
other threads. The testovac doesn't have a good testsuite, we only run it in one
...
For the testing we need to setup an environment. For this purpose I made a
script which will download software on which will be stare2seccomp tested. The
script is is named \texttt{./setEnv} and is located in testsuite folder. The
script has multiple commands:
\begin{itemize}
	\item \texttt{prep} will download, extract and configure tested programs
	\item \texttt{applyPolicy} will patch sourcecodes with generated straces
...
\begin{enumerate}
	...
	computer with x86 architecture.
\end{enumerate}

\subsection{Results}
During testing phase was discovered two big issues both of them during late
development phase. The first issue is caused by libseccomp library which is not
supporting intervals in any way. The libseccomp architecture is specified that
in one fuction \texttt{seccomp\_rule\_add} is logical and among partial
expressions. Among the funstions \texttt{seccomp\_rule\_add} is logical or. In
one function \texttt{seccomp\_rule\_add} can be mentioned only once one argument
position. This means that it is not possible to write something like in Figure
...
This issue was partialy solved in this pull
request\footnote{\url{https://github.com/seccomp/libseccomp/issues/94}}. Problem
in this pull request is that it is not confirmed if it will be in libseccomp.
Because of that in my project is used downstream version of libseccomp which has
to be manualy installed on the system. In the downstreamed version can be
interval expressed in Figure \ref{libseccomp_in_range}.
...
\texttt{SCMP\_CMP\_GT/GE/LT/LE} are not working correctly when negative system
...
this problem is planed to be fixed in next libseccomp release. However in time
of writing this thesis the fix still doesn't exist.

The problem of negative values provided into system call is triaged as important
to fix beacuse in any complex piece of software is used at least one system call
with negative number provided in arguments. Very often it is a \texttt{mmap} or
\texttt{llseek}. The main problem is on the x86\_64 / amd64 architectures. As
you can see the system call numbers are 64bit long numbers and BPF used in
kernel is only 32bits. Libseccomp is comparing 64bit as a two 32bits unsigned
numbers. And this is the problem. This we can proof by libseccomp code or by
disassembling BPF instructions generated by libseccomp.

In Figure \ref{libseccomp_rule} is only rule we are adding into a filter. It
means that fifth arguemnt must be in range $[-2, 3]$ including border values.
...
C/C++ code. On line~6 we store upper 32~bits of a number in accumulator. On
line~7 we are comparing accumulator with value \texttt{0xffffffff} and then on
line~8 with zero. This behaviour is invalid and should be fixed.
...
\begin{figure}[h]
	...
	\end{lstlisting}
	\caption{Disassembled BPF filter}
\end{figure}

These two problems are critical for correct working of strace2seccomp tool.
Because of this it is not possible to test or run the output of strace2seccomp
on amd64 architecture. Only metric for correctness of output on the amd64
architecture is manualy review output.
